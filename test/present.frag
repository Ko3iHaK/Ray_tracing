...
vec4 castRay(inout vec3 ro, inout vec3 rd) {											
	...
	vec3 sphPos = vec3(0.0, 0.0, -0.001);
	...
	sphPos = vec3(0.0, 3.0, -0.001);
	it = sphIntersect(ro - sphPos, rd, 1.0);
	if(it.x > 0.0 && it.x < minIt.x){
		minIt = it;
		vec3 itPos = ro + rd * it.x;
		n = normalize(itPos - sphPos);
		color = vec4(.0, 1.0, 1.0, 0.001);
	}
	
	sphPos = vec3(0.0, 6.0, -0.001);
	it = sphIntersect(ro - sphPos, rd, 1.0);
	if(it.x > 0.0 && it.x < minIt.x){
		minIt = it;
		vec3 itPos = ro + rd * it.x;
		n = normalize(itPos - sphPos);
		color = vec4(1.0, 1.0, 1.0, -0.001);
	}
	
	sphPos = vec3(0.0, 9.0, -0.001);
	it = sphIntersect(ro - sphPos, rd, 1.0);
	if(it.x > 0.0 && it.x < minIt.x){
		minIt = it;
		vec3 itPos = ro + rd * it.x;
